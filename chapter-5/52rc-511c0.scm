...
;		(stack (make-stack))
		(stack-table (map (lambda (reg-name) (list reg-name (make-stack))) register-names))
		(the-instruction-sequence '()))
	(let ((the-ops
		   (list (list 'initialize-stack
					   (lambda () 
						 (map (lambda (named-stack) ((cadr named-stack) 'initialize))
							  stack-table)))))
		  (register-table
...
(define (make-execution-procedure inst labels machine
  ...
  (cond ((eq? (car inst) 'assign)
		...
		((eq? (car inst) 'save)
		 (let ((stack ((machine 'get-stack) (stack-inst-reg-name inst))))
