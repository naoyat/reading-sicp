...
;; based on install-polynomial-package in c253
...
(put 'add '(polynomial scheme-number)
     (lambda (p n) (add (scheme-number->polynomial (car p) n)
                        (attach-tag 'polynomial p))))

(define (render p)
  (myprint ((get 'render 'polynomial) (contents p))))
